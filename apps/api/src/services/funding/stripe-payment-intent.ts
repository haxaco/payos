...
/**
 * Create a Coinbase Offramp session token (sell USDC → fiat).
 */
export async function createOfframpToken(
  input: CreateOnrampTokenInput
): Promise<OnrampTokenResult> {
  const apiKey = CDP_API_KEY();
  const apiSecret = CDP_API_SECRET();
  ...
  if (!apiKey || !apiSecret) {
    return { token: `mock_offramp_token_${Date.now()}` };
  }
  ...
  if (!response.ok) {
    const errorBody = await response.text();
    console.error(`[Coinbase Offramp] API error: ${response.status}`, errorBody);
    throw new Error(`Coinbase Offramp error: ${response.status} - ${errorBody}`);
  }
