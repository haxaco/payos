  ...
  const transactions = [
    ...(ucpCheckouts || []).map((c: any) => {
      ...
      return {
        ...
        status: c.status,
        currency: c.currency,
        // UCP totals are minor units (cents); the unified feed is major
        // units everywhere else (ACP/transfers) — normalize so the agent
        // activity shows $135.00, not 13500.
        amount: (totalLine?.amount || 0) / 100,
        order_id: c.order_id,
