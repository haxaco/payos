...
/**
 * Add compliance flags to existing seed data
 */
...
async function seedComplianceFlags() {
  console.log('🚩 Adding compliance flags...\n');
  ...
  if (!tenant) {
    console.error('❌ Acme Corporation tenant not found');
    process.exit(1);
  }
  ...
  // Get some accounts and transfers
  ...
  if (!accounts || !transfers) {
    console.error('❌ No accounts or transfers found');
    process.exit(1);
  }

  console.log(`✓ Found ${accounts.length} accounts and ${transfers.length} transfers\n`);
  ...
  if (error) {
    console.error('❌ Error inserting flags:', error);
    process.exit(1);
  }

  console.log(`✅ Successfully created ${data?.length || 0} compliance flags:\n`);
  data?.forEach((flag: any) => {
    console.log(`   • ${flag.risk_level.toUpperCase()} - ${flag.reason_code} (${flag.status})`);
  });

  console.log('\n✨ Done! Navigate to http://localhost:5173/compliance to see the flags.\n');
}

seedComplianceFlags().then(() => {
  process.exit(0);
}).catch((error) => {
