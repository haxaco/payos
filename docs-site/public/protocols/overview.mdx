...
- Monetizing **APIs** via micropayments → [x402](/protocols/x402)
...
Protocols compose — you can issue a UCP token that settles on-chain via x402, or accept an AP2 mandate that executes through an ACP checkout. See `/v1/composition` and the [composition guide](/api-reference/openapi) for cross-protocol routing.
