...
Sly also ships an MCP server for the [Scanner service](/api-reference/openapi) — a demand-intelligence tool for merchants and agents.

```bash
npx @sly_ai/scanner-mcp install
```

Exposes 19 tools for scanning merchants, analyzing protocol adoption, generating readiness reports, and running agent shopping tests. See the [scanner docs](/api-reference/openapi) for detail.
...
For non-LLM integrations (backend services, CI, operational scripts), use the [REST API](/api-reference/openapi) directly.
